...
/* ╔═══════════════════════════════════════════════════════════════╗
   ...
   ║  Cambiá estas 3 variables para rediseñar toda la app.        ║
   ║  COLOR_PRIMARY  → titulos, footer, botones principales       ║
   ║  COLOR_SECONDARY → acento: precios, tags, detalles dorados   ║
   ║  COLOR_TERTIARY  → fondo principal claro                     ║
   ╚═══════════════════════════════════════════════════════════════╝ */

const COLOR_PRIMARY   = "#1b3a2e";   // Verde bosque oscuro (títulos, botones, footer)
const COLOR_SECONDARY = "#c8973b";   // Dorado cálido clásico (precios, acentos)
const COLOR_TERTIARY  = "#faf7f0";   // Crema cálida (fondo día)

// ── MODO DÍA — paleta clásica crema + verde + dorado ─────────
const THEME_LIGHT = {
  bg:        COLOR_TERTIARY,          // Crema principal
  surface:   "#f7f2e8",               // Crema secundaria / secciones
  card:      "#ede5d4",               // Cards, inputs
  primary:   COLOR_PRIMARY,           // Verde bosque oscuro
  accent:    COLOR_SECONDARY,         // Dorado clásico
  text:      "#1a1a14",               // Texto principal
  muted:     "#5a6a5e",               // Texto secundario
  border:    "rgba(27,58,46,0.15)",   // Borde verde sutil
  success:   "#2d5e45",               // Verde medio
  danger:    "#c84a3a",               // Rojo
  info:      "#3a5a8a",               // Azul info
};

// ── MODO NOCHE — versión oscura de la misma paleta verde ──────
const THEME_DARK = {
  bg:        "#0f1f18",               // Verde muy oscuro casi negro
  surface:   "#162b1f",               // Verde oscuro profundo
  card:      "#1e3a2c",               // Verde bosque oscuro
  primary:   "#4a8c68",               // Verde claro (contrasta sobre oscuro)
  accent:    "#e8b85a",               // Dorado más brillante en oscuro
  text:      "#e8e4d8",               // Crema suave
  muted:     "#8aaa96",               // Verde grisáceo
  border:    "rgba(74,140,104,0.2)",  // Borde verde tenue
  success:   "#6aaa88",               // Verde éxito
  danger:    "#e05a4a",               // Rojo
  info:      "#7aaae0",               // Azul info
};

// ── TEMA ACTIVO (se actualiza dinámicamente) ──────────────────
let C = { ...THEME_LIGHT };

// Aplica el tema como CSS variables en :root
function applyTheme(theme) {
  Object.assign(C, theme);
  const r = document.documentElement;
  r.style.setProperty("--luma-bg",      theme.bg);
  r.style.setProperty("--luma-surface", theme.surface);
  r.style.setProperty("--luma-card",    theme.card);
  r.style.setProperty("--luma-primary", theme.primary);
  r.style.setProperty("--luma-accent",  theme.accent);
  r.style.setProperty("--luma-text",    theme.text);
  r.style.setProperty("--luma-muted",   theme.muted);
  r.style.setProperty("--luma-border",  theme.border);
  r.style.setProperty("--luma-success", theme.success);
  r.style.setProperty("--luma-danger",  theme.danger);
  r.style.setProperty("--luma-info",    theme.info);
}
...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  body{background:var(--luma-bg);color:var(--luma-text);transition:background .4s ease,color .3s ease}
  ::-webkit-scrollbar{width:5px}
  ::-webkit-scrollbar-track{background:var(--luma-bg)}
  ::-webkit-scrollbar-thumb{background:var(--luma-primary);border-radius:3px}
  @keyframes fadeUp{from{opacity:0;transform:translateY(24px)}to{opacity:1;transform:translateY(0)}}
  @keyframes fadeIn{from{opacity:0}to{opacity:1}}
  @keyframes slideRight{from{transform:translateX(100%)}to{transform:translateX(0)}}
  @keyframes spin{to{transform:rotate(360deg)}}
  @keyframes themeToggle{0%{transform:scale(1)}50%{transform:scale(1.15)}100%{transform:scale(1)}}
  .fade-up{animation:fadeUp .5s ease both}
  .fade-in{animation:fadeIn .4s ease both}
  .slide-right{animation:slideRight .3s cubic-bezier(.4,0,.2,1)}
  .hover-card:hover{border-color:var(--luma-primary) !important;transform:translateY(-4px);box-shadow:0 16px 40px rgba(147,171,217,0.2)}
  .hover-card{transition:all .25s ease}
  .spin{animation:spin .8s linear infinite;display:inline-block}
  ::selection{background:var(--luma-accent);color:var(--luma-bg)}
  .luma-transition *{transition:background .4s ease,border-color .3s ease,color .3s ease !important}
`;

/* ═══════════════════════════════════════════════════════
   SHARED UI
═══════════════════════════════════════════════════════ */
// inp and lbl are computed inline via getInp()/getLbl() to support live theme switching
const getInp = () => ({ background: C.card, border: `2px solid ${C.border}`, color: C.text, padding: "11px 14px", fontFamily: "'Outfit',sans-serif", fontSize: 14, width: "100%", outline: "none", borderRadius: 10 });
const getLbl = () => ({ display: "block", fontSize: 11, fontWeight: 600, letterSpacing: 1.2, color: C.primary, marginBottom: 6, textTransform: "uppercase", fontFamily: "'Space Mono',monospace" });

function Spinner() { return <span className="spin" style={{ fontSize: 16 }}>⟳</span>; }

function Btn({ children, v = "primary", onClick, style = {}, disabled }) {
  const vars = {
    primary: { background: C.primary, color: C.bg, border: "none" },
...
function Field({ label, children }) { return <div style={{ marginBottom: 16 }}>{label && <label style={getLbl()}>{label}</label>}{children}</div>; }
function Input({ label, ...p }) { return <Field label={label}><input style={getInp()} {...p} /></Field>; }
function Textarea({ label, ...p }) { return <Field label={label}><textarea style={{ ...getInp(), resize: "vertical", minHeight: 90 }} {...p} /></Field>; }
...
function Modal({ title, onClose, children, width = 520 }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(30,30,60,0.55)", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
      ...
      <div className="fade-in" style={{ background: C.surface, border: `1px solid ${C.border}`, width: "100%", maxWidth: width, maxHeight: "90vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 24px", borderBottom: `1px solid ${C.border}` }}>
          <h3 style={{ fontFamily: "'Fraunces',serif", fontSize: 22, color: C.primary }}>{title}</h3>
...
function Toast({ toast }) {
  ...
  return <div className="fade-in" style={{ position: "fixed", top: 20, right: 20, zIndex: 999, padding: "14px 22px", background: { success: C.success, error: C.danger, info: C.info }[toast.type] || C.primary, color: "#2a2a1a", fontFamily: "'Outfit',sans-serif", fontWeight: 600, fontSize: 14, boxShadow: "0 8px 32px rgba(147,171,217,0.4)", maxWidth: 340 }}>{toast.msg}</div>;
}
...
function TeamSection() {
  ...
  const employees  = TEAM.slice(6);

  const MemberCard = ({ m }) => (
    <div className="hover-card" style={{ background: C.surface, border: `1px solid ${C.border}`, padding: "20px 14px", textAlign: "center", borderRadius: 16, display: "flex", flexDirection: "column", alignItems: "center" }}>
      <div style={{ width: 58, height: 58, borderRadius: "50%", background: `${m.color}18`, border: `2px solid ${m.color}45`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 12, fontSize: 13, fontWeight: 700, color: m.color, fontFamily: "'Fraunces',serif", letterSpacing: "0.04em", flexShrink: 0 }}>{m.ini}</div>
      <div style={{ fontWeight: 600, color: C.primary, fontSize: 12, marginBottom: 4, lineHeight: 1.3 }}>{m.name}</div>
      <div style={{ fontSize: 10, color: C.accent, lineHeight: 1.4, marginBottom: 3 }}>{m.role}</div>
      <div style={{ fontSize: 9, color: C.muted, fontFamily: "'Space Mono',monospace", letterSpacing: "0.06em", textTransform: "uppercase" }}>{m.area}</div>
    </div>
  );
    ...
    <section style={{ background: C.bg, padding: "90px 28px", borderTop: `1px solid ${C.border}` }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 52 }}>
          <Tag>El equipo</Tag>
          <h2 style={{ fontFamily: "'Fraunces',serif", fontSize: 48, margin: "20px 0 12px", color: C.primary }}>El equipo detrás de Luma</h2>
          <p style={{ color: C.muted, fontSize: 14, fontFamily: "'Space Mono',monospace", letterSpacing: "0.08em" }}>// {TEAM.length} personas · {[...new Set(TEAM.map(m=>m.area))].length} áreas</p>
        </div>

        {/* Leadership row — always visible */}
        <div className="team-grid" style={{ marginBottom: 14 }}>
          {leadership.map((m, i) => <MemberCard key={i} m={m} />)}
        </div>

        {/* Employees — collapsible */}
        <div style={{ overflow: "hidden", maxHeight: expanded ? 2000 : 0, transition: "max-height 0.5s cubic-bezier(0.4,0,0.2,1)" }}>
          <div className="team-grid" style={{ paddingTop: 14 }}>
            {employees.map((m, i) => <MemberCard key={i} m={m} />)}
          </div>
        </div>
        ...
        <div style={{ textAlign: "center", marginTop: 32 }}>
          <button
            ...
            style={{ background: "transparent", border: `2px solid #93ABD9`, color: C.primary, padding: "12px 36px", fontFamily: "'Outfit',sans-serif", fontWeight: 600, fontSize: 14, borderRadius: 100, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 10, transition: "all .25s" }}
            onMouseOver={e => { e.currentTarget.style.background = C.primary; e.currentTarget.style.color = C.bg; }}
            onMouseOut={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = C.primary; }}>
            {expanded
              ? <><span style={{ transition: "transform .3s", display: "inline-block", transform: "rotate(180deg)" }}>↓</span> Ver menos</>
              : <><span>↓</span> Ver los {employees.length} empleados</>}
          </button>
        </div>
      </div>
    </section>
...
/* ═══════════════════════════════════════════════════════
   NAVBAR
═══════════════════════════════════════════════════════ */
function Navbar({ page, navigate, cartCount, darkMode, onToggleTheme }) {
  return (
    <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "var(--luma-surface)", backdropFilter: "blur(12px)", borderBottom: `1px solid ${C.border}` }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 62 }}>
        <button onClick={() => navigate("home")} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontFamily: "'Fraunces',serif", fontSize: 26, color: C.primary, fontWeight: 800, letterSpacing: "-0.02em" }}>Luma<span style={{ color: C.accent }}>✦</span></span>
        </button>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {[["home","Inicio"],["shop","Tienda"],["qr","Ver código"],].map(([id, label]) => (
          ...
          <button
            ...
            style={{ background: darkMode ? "#ffffff18" : "#00000010", border: `1px solid ${C.border}`, color: C.muted, width: 36, height: 36, borderRadius: "50%", cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", marginLeft: 6, transition: "all .25s", flexShrink: 0 }}
...
function LandingPage({ navigate }) {
  const features = [
    { e: "✉️", t: "Mensaje personalizado", d: "Escribí lo que querés que vea tu destinatario al escanear el código" },
  ...
  return (
    <div>
      <section style={{ minHeight: "92vh", display: "flex", alignItems: "center", position: "relative", overflow: "hidden", padding: "0 28px", background: `linear-gradient(135deg,${C.surface} 0%,${C.bg} 60%)` }}>
        ...
        <div style={{ position: "absolute", right: "-10%", top: "-5%", width: "55%", height: "110%", background: C.primary, borderRadius: "60% 0 0 50%", zIndex: 0, clipPath: "ellipse(60% 90% at 100% 50%)", opacity: 0.08 }} />
        <div style={{ maxWidth: 1200, margin: "0 auto", width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          <div className="fade-up">
            ...
            <h1 style={{ fontFamily: "'Fraunces',serif", fontSize: "clamp(3rem,5.5vw,5rem)", fontWeight: 800, lineHeight: 1.02, color: C.primary, marginBottom: 22, letterSpacing: "-0.03em" }}>Cuadernos que<br /><em style={{ color: C.accent, fontStyle: "italic", fontWeight: 300 }}>inspiran</em><br />a aprender</h1>
            <p style={{ fontSize: 16, color: C.muted, lineHeight: 1.75, marginBottom: 36, maxWidth: 460 }}>Cuadernos inteligentes hechos con creatividad, organización y conciencia. Diseñados por estudiantes de 5to año para ayudarte a estudiar mejor, de forma más divertida y creativa.</p>
            <div style={{ display: "flex", gap: 14 }}>
              <Btn onClick={() => navigate("shop")} style={{ padding: "14px 32px", fontSize: 14 }}>Ver productos →</Btn>
              <Btn v="outline" onClick={() => navigate("qr")} style={{ padding: "14px 28px", fontSize: 14 }}>Probar Código</Btn>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {features.map((f, i) => (
              <div key={i} className="hover-card fade-up" style={{ background: C.bg, border: `1px solid ${C.border}`, padding: 24, animationDelay: `${i * 0.1}s`, borderRadius: 16 }}>
                ...
                <div style={{ fontWeight: 600, color: C.primary, marginBottom: 8, fontSize: 15 }}>{f.t}</div>
      ...
      <section style={{ background: C.surface, padding: "80px 28px", borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 820, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "'Fraunces',serif", fontSize: 48, margin: "20px 0 28px", color: C.primary }}>Cuadernos inteligentes<br /><em style={{ color: C.accent, fontStyle: "italic" }}>hechos con amor</em></h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24, marginTop: 52 }}>
            {[{ n:"5to", l:"Año escolar"}, { n:"100%", l:"Creatividad y conciencia"}, { n:"✦", l:"Ideas que brillan"}].map((s,i)=>(
              <div key={i} style={{ padding: 28, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16 }}>
                <div style={{ fontFamily: "'Fraunces',serif", fontSize: 44, color: C.primary, fontWeight: 800 }}>{s.n}<span style={{ color: C.accent }}>.</span></div>
      ...
      <section style={{ padding: "90px 28px", textAlign: "center", background: C.primary }}>
        <blockquote style={{ fontFamily: "'Fraunces',serif", fontSize: 40, fontStyle: "italic", color: C.bg, maxWidth: 750, margin: "0 auto 20px", lineHeight: 1.4 }}>
          "Ideas que brillan."
        </blockquote>
        <p style={{ color: `${C.bg}99`, letterSpacing: 2, textTransform: "uppercase", fontSize: 11, fontFamily: "'Space Mono',monospace" }}>— Lema Luma</p>
      </section>
      ...
      <section style={{ background: C.surface, padding: "80px 28px", borderTop: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            ...
            <h2 style={{ fontFamily: "'Fraunces',serif", fontSize: 42, color: C.primary, fontWeight: 700, letterSpacing: "-0.02em" }}>Hablemos <em style={{ fontStyle: "italic", fontWeight: 300, color: C.accent }}>hoy</em></h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 20 }}>
            {[
              ...
              <a key={i} href={c.href} target={c.href.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer"
                ...
                onMouseOver={e => { e.currentTarget.style.transform = "translateX(6px)"; e.currentTarget.style.boxShadow = "0 8px 24px rgba(147,171,217,0.3)"; }}
                ...
                <div>
                  ...
                  <div style={{ fontWeight: 600, color: C.primary, fontSize: 14 }}>{c.value}</div>
                </div>
              </a>
            ))}
          </div>
      ...
      <footer style={{ background: C.primary, borderTop: `1px solid ${C.border}`, padding: "36px 28px", textAlign: "center" }}>
        <p style={{ fontFamily: "'Fraunces',serif", fontSize: 26, color: C.accent, marginBottom: 10, fontWeight: 800 }}>Luma<span style={{ color: C.accent }}>✦</span></p>
        <p style={{ fontSize: 13, color: `${C.bg}cc` }}>Cuadernos inteligentes hechos con creatividad, organización y conciencia.</p>
        <p style={{ fontSize: 12, color: `${C.bg}80`, marginTop: 16 }}>© 2025 Luma — Emprendimiento de 5to año · Buenos Aires</p>
      </footer>
    </div>
  );
}
...
function CheckoutModal({ cart, coupons, onClose, onSuccess, showToast }) {
  ...
  const submit = async () => {
    ...
    for (const item of cart) {
      const fresh = await db.products.list().then(ps => ps.find(p => p.id === item.id));
      if (!fresh) { showToast(`El producto "${item.name}" ya no existe.`, "error"); return; }
      if (fresh.stock < item.qty) {
        showToast(`Stock insuficiente para "${item.name}" — quedan ${fresh.stock} u.`, "error");
        return;
      }
    }
    setSending(true);
    try {
      ...
      // Descontar stock en Supabase — usamos stock actual de BD (ya validado arriba)
      const freshStocks = await db.products.list();
      await Promise.all(
        cart.map(item => {
          const current = freshStocks.find(p => p.id === item.id);
          const newStock = Math.max(0, (current?.stock ?? item.stock) - item.qty);
          return db.products.update(item.id, { stock: newStock });
        })
      );
      ...
      onSuccess(cart.map(i => ({ id: i.id, qty: i.qty })));
  ...
  return (
    <Modal title="Confirmar pedido" onClose={onClose} width={560}>
      ...
      <div style={{ marginBottom: 16 }}>
        <p style={{ ...getLbl(), marginBottom: 10 }}>Resumen del pedido</p>
        {cart.map(i => (
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <input value={couponInput} onChange={e => setCouponInput(e.target.value)} placeholder="Código de cupón..." style={{ ...getInp(), flex: 1 }} />
...
/* ═══════════════════════════════════════════════════════
   SHOP
═══════════════════════════════════════════════════════ */
function ShopPage({ products, setProducts, loading, cart, setCart, coupons, showToast }) {
  ...
  const updateQty = (key, qty) => {
    ...
    setCart(prev => {
      ...
      if (item && qty > item.stock) {
        showToast(`Máximo ${item.stock} u. disponibles`, "error");
        return prev;
      }
      return prev.map(i => i._key === key ? { ...i, qty } : i);
    });
  };
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
      <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "36px 28px", boxShadow: "0 2px 20px rgba(147,171,217,0.2)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            <Tag>Catálogo</Tag>
            <h1 style={{ fontFamily: "'Fraunces',serif", fontSize: 42, color: C.primary, marginTop: 12, fontWeight: 800 }}>Nuestra Tienda</h1>
          </div>
            ...
            {products.map((p, idx) => (
              <div key={p.id} className="hover-card fade-up" style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 20, display: "flex", flexDirection: "column", animationDelay: `${idx * 0.08}s`, overflow: "hidden" }}>
                <div style={{ height: 210, position: "relative", overflow: "hidden", borderBottom: `1px solid ${C.border}`, background: C.card }}>
                  ...
                  <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top,rgba(147,171,217,0.25) 0%,transparent 60%)", pointerEvents: "none" }} />
                ...
                <div style={{ padding: 24, flex: 1, display: "flex", flexDirection: "column" }}>
                  ...
                  <h3 style={{ fontFamily: "'Fraunces',serif", fontSize: 22, color: C.primary, marginBottom: 10, fontWeight: 700 }}>{p.name}</h3>
                  ...
                  <div style={{ marginTop: "auto" }}>
                    <div style={{ marginBottom: 12 }}>
                      ...
                      <div style={{ fontSize: 28, fontWeight: 800, color: C.accent, fontFamily: "'Fraunces',serif", lineHeight: 1.1 }}>${finalPrice(p).toLocaleString("es-AR")}</div>
                      <div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>Stock: {p.stock} u.</div>
                    </div>
                    <Btn onClick={() => handleAddClick(p)} disabled={p.stock === 0} style={{ width: "100%", justifyContent: "center", padding: "12px 16px", fontSize: 13, borderRadius: 100 }}>{p.stock === 0 ? "Sin stock" : p.custom ? "Personalizar →" : "+ Agregar"}</Btn>
                  </div>
                </div>
              </div>
            ))}
      ...
      {cartOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 300 }}>
          ...
          <div className="slide-right" style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 440, background: C.bg, borderLeft: `1px solid ${C.border}`, display: "flex", flexDirection: "column", boxShadow: "-20px 0 60px rgba(147,171,217,0.25)" }}>
            <div style={{ padding: "22px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h2 style={{ fontFamily: "'Fraunces',serif", fontSize: 26, color: C.primary }}>Tu carrito</h2>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
              {cart.length === 0 ? (
                ...
                <div key={item._key} style={{ display: "flex", gap: 14, marginBottom: 16, padding: 16, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12 }}>
                  <div style={{ width: 54, height: 54, flexShrink: 0, overflow: "hidden", background: C.card }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, color: C.primary, fontSize: 14, marginBottom: 2 }}>{item.name}</div>
                    {item.customization && (
      ...
      {checkoutOpen && (
        <CheckoutModal
          ...
          onSuccess={(soldItems) => {
            // Update local product stock immediately (optimistic UI)
    ...
    return (
      <div className="fade-in" style={{ position: "fixed", inset: 0, zIndex: 200, overflowY: "auto", background: C.bg }}>
        ...
        <div style={{ position: "sticky", top: 0, zIndex: 10, background: `${C.bg}e0`, backdropFilter: "blur(12px)", borderBottom: `1px solid ${C.border}`, padding: "14px 28px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ fontFamily: "'Fraunces',serif", color: C.primary, fontSize: 18, fontWeight: 800 }}>Luma<span style={{ color: C.accent }}>✦</span></span>
        ...
        <div style={{ minHeight: "calc(100vh - 57px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "60px 28px" }}>
          {result.type === "mensaje" && (
            <div className="fade-up" style={{ maxWidth: 720, width: "100%", textAlign: "center" }}>
              ...
              <p style={{ fontFamily: "'Fraunces',serif", fontSize: "clamp(28px,5vw,52px)", fontStyle: "italic", color: C.primary, lineHeight: 1.65 }}>"{result.content}"</p>
          ...
          {result.type === "versiculo" && (
            <div className="fade-up" style={{ maxWidth: 780, width: "100%", textAlign: "center" }}>
              ...
              <p style={{ fontFamily: "'Fraunces',serif", fontSize: "clamp(28px,5vw,52px)", fontStyle: "italic", color: C.primary, lineHeight: 1.6, marginBottom: 32 }}>"{result.content.replace(/ — .*/,"")}"</p>
          ...
          {result.type === "aleatorio" && (() => {
            ...
            return (
              <div className="fade-up" style={{ maxWidth: 780, width: "100%", textAlign: "center" }}>
                ...
                <div style={{ width: 80, height: 1, background: `linear-gradient(to right,transparent,${C.accent},transparent)`, margin: "0 auto 52px" }} />
                <p style={{ fontFamily: "'Fraunces',serif", fontSize: "clamp(28px,5vw,52px)", fontStyle: "italic", color: C.primary, lineHeight: 1.65, marginBottom: 32 }}>"{v.text}"</p>
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "64px 24px" }}>
      <div style={{ maxWidth: 520, width: "100%" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          ...
          <h2 style={{ fontFamily: "'Fraunces',serif", fontSize: 36, color: C.primary, marginBottom: 10 }}>Ver mi mensaje</h2>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, padding: 28 }}>
          <div style={{ display: "flex", gap: 10 }}>
            <input value={input} onChange={e => { setInput(e.target.value); setSearched(false); }} onKeyDown={e => e.key === "Enter" && search()} placeholder="Ej: A123456789" style={{ ...getInp(), flex: 1, fontSize: 16, textTransform: "uppercase", letterSpacing: 1.5 }} />
        ...
        {searched && !result && (
          <div className="fade-up" style={{ marginTop: 20, background: C.surface, border: `1px solid ${C.border}`, padding: 36, textAlign: "center" }}>
            ...
            <p style={{ fontWeight: 700, color: C.primary, fontSize: 16, marginBottom: 8 }}>Código no encontrado</p>
    ...
    <div style={{ minHeight: "100vh", padding: "52px 28px" }}>
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, padding: 24, marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <input value={code} onChange={e => { setCode(e.target.value); setSearched(false); }} onKeyDown={e => e.key === "Enter" && doSearch()} placeholder="Ej: ANO-001" style={{ ...getInp(), flex: 1, fontSize: 16, textTransform: "uppercase", letterSpacing: 1 }} />
          ...
          <div className="fade-up" style={{ background: C.surface, border: `1px solid ${C.border}`, padding: 24, marginBottom: 20 }}>
            ...
            <Field label={typeConfig[type].label}>
              {type === "aleatorio" ? (
                ...
                <textarea value={content} onChange={e => setContent(e.target.value)} rows={type==="video"?2:5} placeholder={typeConfig[type].ph} style={{ ...getInp(), resize:"vertical", minHeight:type==="video"?52:110 }} />
              )}
            </Field>
          ...
          <div>
            <div style={{ background:C.surface, border:`1px solid ${C.border}`, padding:24, marginBottom:24 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 160px auto", gap:12, alignItems:"flex-end" }}>
                <Field label="Código"><input value={cpForm.code} onChange={e=>setCpForm(f=>({...f,code:e.target.value}))} placeholder="Ej: VERANO25" style={{...getInp(), textTransform:"uppercase", letterSpacing:1}}/></Field>
                <Field label="Descuento (%)"><input type="number" min={1} max={100} value={cpForm.discount} onChange={e=>setCpForm(f=>({...f,discount:e.target.value}))} style={getInp()}/></Field>
          ...
          <Field label="Imagen del producto">
            <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
              ...
              <div style={{flex:1}}>
                ...
                <input value={form.img_url} onChange={e=>setF("img_url",e.target.value)} placeholder="O pegá una URL de imagen aquí" style={{...getInp(),fontSize:12,padding:"8px 12px"}}/>
              </div>
            </div>
          </Field>
          <Divider/>
...
export default function App() {
  ...
  useEffect(() => { applyTheme(THEME_LIGHT); }, []);
  ...
  return (
    <div style={{ fontFamily: "'Outfit',sans-serif", background: C.bg, color: C.text, minHeight: "100vh", lineHeight: 1.6 }}>
      <style>{css}</style>
      <Toast toast={toast} />
      <Navbar page={page} navigate={navigate} cartCount={cartCount} darkMode={darkMode} onToggleTheme={toggleTheme} />
      {page === "home"  && <LandingPage navigate={navigate} />}
      {page === "shop"  && <ShopPage products={products} setProducts={setProducts} loading={loadingShop} cart={cart} setCart={setCart} coupons={coupons} showToast={showToast} />}
